...
//        cyphertext[15] = _mm_aesenclast_si128(cyphertext[15], mRoundKey[10]);
    ...
    {
        //assin zero to the array of indices which are set as the plaintext. Note that we only use the list significant long part of each 128 bit.
        u64 *plaintextArray = new u64[2*blockLength];

        //go over the array and set the 64 list significant bits for evey 128 bit value, we use only half of the 128 bit variables
        for (u64 i = 0; i < blockLength; i++, baseIdx++) {
            plaintextArray[2*i + 1] = baseIdx;
            plaintextArray[2*i] = 0;
        }
        int size;
        // Compute aes on the given plaintext
	    EVP_EncryptUpdate(opensslAes, (byte*)cyphertext, &size, (byte*)plaintextArray, 16*blockLength);
